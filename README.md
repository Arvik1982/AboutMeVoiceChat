...
Форма обратной связи отправляет письма через **Resend API**. 
В целях демонстрации используется отправитель `onboarding@resend.dev` (официальный тестовый адрес Resend). 
Для production-проектов достаточно верифицировать свой домен (или email через Single Sender Verification), чтобы отправлять письма с личного адреса.
